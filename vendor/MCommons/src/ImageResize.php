...
class ImageResize
{
    ...
    function __construct($fileName)
    {
        ...
        if ($this->testIsImage($this->image)) {
            ...
            /*
             * Added 15-09-08 Get the filesize using this build in method. Stores an array of size $this->imageSize[1] = width $this->imageSize[2] = height $this->imageSize[3] = width x height
             */
    ...
    //
    {
        ...
        imagecopymerge($this->imageResized, $cornerImg, 0, 0, 0, 0, $radius, $radius, 100); // l
                                                                                            
        // *** Map rounded corner to other corners by rotating and applying the mask
        $cornerImg = imagerotate($cornerImg, 90, 0);
        imagecopymerge($this->imageResized, $cornerImg, 0, $this->height - $radius, 0, 0, $radius, $radius, 100); // l
        
        $cornerImg = imagerotate($cornerImg, 90, 0);
        imagecopymerge($this->imageResized, $cornerImg, $this->width - $radius, $this->height - $radius, 0, 0, $radius, $radius, 100); // r
        
        $cornerImg = imagerotate($cornerImg, 90, 0);
        imagecopymerge($this->imageResized, $cornerImg, $this->width - $radius, 0, 0, 0, $radius, $radius, 100); // r
        ...
        while ($Y >= 0) {
            ...
            while ($X < $BMP['width']) {
                ...
                elseif ($BMP['bits_per_pixel'] == 16) {
                    ...
                    /*
                     * BMP 16bit fix ================= Ref: http://us3.php.net/manual/en/function.imagecreate.php#81604 Notes: "don't work with bmp 16 bits_per_pixel. change pixel generator for this."
                     */
